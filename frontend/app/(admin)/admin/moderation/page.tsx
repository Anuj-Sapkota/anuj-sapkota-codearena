...
import { MdShield, MdCheckCircle, MdBlock, MdWarning, MdPerson } from "react-icons/md";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
          <h1 className="text-3xl font-black text-slate-900 flex items-center gap-3 tracking-tight">
            <MdShield className="text-blue-600" /> MODERATION_CENTER
          </h1>
          <p className="text-slate-500 font-medium mt-1">Reviewing flagged content for community safety.</p>
        </div>
        <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-2xl shadow-sm border border-slate-200">
          ...
          <span className="font-bold text-slate-700">{flaggedItems.length} Pending Incidents</span>
        </div>
      </div>
      ...
      <div className="grid gap-6">
        {flaggedItems.length === 0 ? (
          <div className="bg-white border-2 border-dashed border-slate-200 rounded-3xl p-20 text-center">
            ...
            <p className="text-slate-400 font-medium">No discussions currently meet the report threshold.</p>
          </div>
        ) : (
          flaggedItems.map((item: any) => (
            <div key={item.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col md:flex-row">
              <div className="bg-slate-900 p-8 flex flex-col items-center justify-center text-white min-w-[160px]">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest opacity-60">Reports</span>
                <div className="mt-6 flex flex-col gap-2 w-full">
                  <button onClick={() => handleModerate(item.id, "UNBLOCK")}
                    className="flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white py-2 rounded-xl transition-all">
                    <MdCheckCircle size={18} />
                  </button>
                  <button onClick={() => handleModerate(item.id, "BLOCK")} disabled={item.isBlocked}
                    className={`flex items-center justify-center gap-2 py-2 rounded-xl transition-all ${item.isBlocked ? "bg-slate-700 cursor-not-allowed opacity-50" : "bg-rose-500 hover:bg-rose-600"}`}>
                    <MdBlock size={18} />
                  </button>
                </div>
              </div>
              ...
              <div className="p-8 flex-1">
                <div className="flex items-center gap-3 mb-4">
                  <div className="p-2 bg-slate-100 rounded-lg text-slate-500"><MdPerson size={20} /></div>
                  <div>
                    <p className="text-sm font-bold text-slate-900">@{item.user?.username || "anonymous"}</p>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {item.reports?.map((report: any, idx: number) => (
                      <div key={idx} className="bg-white border border-slate-100 p-3 rounded-xl flex flex-col gap-1 shadow-sm">
                        ...
                        <span className="text-[9px] text-slate-400 font-bold">BY: @{report.user?.username}</span>
                      </div>
                    ))}
                  </div>
                </div>
