...
import {
  FaPlus, FaTrashAlt, FaFolderOpen, FaChartLine,
  FaSpinner, FaEdit, FaSearch, FaChevronLeft, FaChevronRight, FaSlidersH,
} from "react-icons/fa";
...
export default function AdminCategoriesPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (search) result = result.filter((c: Category) => c.name.toLowerCase().includes(search.toLowerCase()) || c.slug.toLowerCase().includes(search.toLowerCase()));
  ...
  const handleDelete = (cat: Category) => {
    if (window.confirm(`CONFIRM_DELETION: ${cat.name.toUpperCase()}?`))
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-200 pb-8 px-2 sm:px-0">
        <div>
          <h1 className="text-4xl font-black text-darkest tracking-tight uppercase">Categories<span className="text-primary-1">.</span></h1>
          <p className="text-muted text-sm font-medium mt-1 uppercase tracking-widest">System Taxonomy Management</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 px-2 sm:px-0">
        <div className="bg-white p-6 border-2 border-gray-200 rounded-md flex items-center justify-between shadow-sm">
          <div><p className="text-muted text-[10px] font-black uppercase tracking-[0.2em]">Total</p><p className="text-3xl font-black text-darkest mt-1 italic">{categories.length}</p></div>
          <div className="h-14 w-14 bg-gray-50 rounded-md border border-gray-100 flex items-center justify-center text-primary-1"><FaFolderOpen size={24} /></div>
        </div>
        <div className="bg-white p-6 border-2 border-gray-200 rounded-md flex items-center justify-between shadow-sm">
          <div><p className="text-muted text-[10px] font-black uppercase tracking-[0.2em]">Usage</p><p className="text-3xl font-black text-darkest mt-1 italic">{categories.reduce((acc: number, c: Category) => acc + (c._count?.problems || 0), 0)}</p></div>
          <div className="h-14 w-14 bg-gray-50 rounded-md border border-gray-100 flex items-center justify-center text-darkest"><FaChartLine size={24} /></div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 px-2 sm:px-0">
        ...
        <button onClick={() => setIsFilterOpen(true)}
          ...
          {(filters.sortBy !== "name_asc" || filters.hasProblems !== "all") && <span className="h-2 w-2 bg-primary-1 rounded-full" />}
        </button>
      </div>

      <div className="bg-white border-2 border-gray-200 rounded-md overflow-hidden mx-2 sm:mx-0 shadow-sm">
        {isLoading ? (
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-muted font-bold block mt-0.5 line-clamp-1 max-w-xs uppercase">{cat.description || "No description"}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5 hidden sm:table-cell font-code text-[11px] text-primary-1">/{cat.slug}</td>
                    <td className="px-6 py-5 text-center font-black text-darkest tabular-nums">{cat._count?.problems || 0}</td>
                    <td className="px-6 py-5">
                      <div className="flex justify-end gap-3">
                        <button onClick={() => { setEditingCategory(cat); setIsModalOpen(true); }}
                          className="h-10 w-10 border-2 border-gray-200 flex items-center justify-center text-muted hover:text-primary-1 hover:border-primary-1/40 rounded-md transition-all cursor-pointer">
                          <FaEdit size={16} />
                        </button>
                        <button onClick={() => handleDelete(cat)}
                          className="h-10 w-10 border-2 border-gray-200 flex items-center justify-center text-muted hover:text-red-500 hover:border-red-200 rounded-md transition-all cursor-pointer">
                          <FaTrashAlt size={16} />
                        </button>
                      </div>
                    </td>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white border-2 border-gray-200 p-4 rounded-md mx-2 sm:mx-0 shadow-sm">
        <p className="text-[10px] font-black text-muted uppercase tracking-widest">{filteredItems.length} Categories found</p>
      ...
      <CreateCategoryModal key={editingCategory?.categoryId || "new"} isOpen={isModalOpen}
        onClose={() => { setIsModalOpen(false); setEditingCategory(null); }} editData={editingCategory} />
      <CategoryFilterModal isOpen={isFilterOpen} onClose={() => setIsFilterOpen(false)} filters={filters} setFilters={setFilters} />
    </div>
  );
}
