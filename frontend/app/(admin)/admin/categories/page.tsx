  ...
  export default function AdminCategoriesPage() {
    ...
    // NEW: Refined Delete Handler with Error Catching
    const handleDelete = async (cat: Category) => {
      const confirmDelete = window.confirm(
        `CONFIRM_DELETION: ${cat.name.toUpperCase()}?`,
      );
      if (!confirmDelete) return;

      try {
        await dispatch(deleteCategoryThunk(cat.categoryId)).unwrap();
        toast.success("CATEGORY_REMOVED_SUCCESSFULLY");
      } catch (err: unknown) {
        const errorMessage =
    ...
    return (
      <div className="space-y-8 max-w-7xl mx-auto pb-10">
        ...
        <div className="flex flex-col md:flex-row gap-4 px-2 sm:px-0">
          ...
          >
            <FaSlidersH
              className="text-muted group-hover:text-primary-1 transition-colors"
              size={14}
            />
            <span className="text-xs font-black text-muted uppercase tracking-widest group-hover:text-darkest">
              Filters
            </span>
        ...
        <div className="bg-white border-2 border-gray-200 rounded-md overflow-hidden mx-2 sm:mx-0 shadow-sm">
          {isLoading && categories.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-80">
              ...
              <p className="text-muted text-[10px] font-black uppercase mt-4 tracking-widest">
                Fetching Data
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-gray-50/80 border-b-2 border-gray-200">
                    <th className="px-6 py-5 text-[10px] font-black text-muted uppercase tracking-widest">
                      Details
                    </th>
                    <th className="px-6 py-5 text-[10px] font-black text-muted uppercase tracking-widest hidden sm:table-cell">
                      Slug
                    </th>
                    <th className="px-6 py-5 text-[10px] font-black text-muted uppercase tracking-widest text-center">
                      Count
                    </th>
                    <th className="px-6 py-5 text-[10px] font-black text-muted uppercase tracking-widest text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
                    ...
                    >
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <span className="font-black text-darkest uppercase text-sm block tracking-tight">
                              {cat.name}
                            </span>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex justify-end gap-3">
                          ...
                          {/* UPDATED: Calling the new handleDelete function */}
                          <button
