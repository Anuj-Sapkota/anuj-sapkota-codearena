...
function SortableLesson({ lesson, onRemove, onUpdate, uploadingId, onUploadFile }: {
  ...
  return (
    <div ref={setNodeRef} style={style} className="border border-slate-200 rounded-lg bg-white shadow-sm overflow-hidden">
      {/* Row header */}
        ...
        <div className="border-t border-slate-100 px-4 py-4 space-y-4 bg-slate-50/50">
          ...
          {lesson.fileType === "code" && lesson.codeContent && (
            <div>
              ...
              <div className="rounded-lg overflow-hidden border border-slate-200 max-h-64 overflow-y-auto text-xs">
      ...
      {!collapsed && (
        <div className="p-4 space-y-2">
          ...
          <button type="button" onClick={() => onAddLesson(section.id)}
            className="flex items-center gap-2 w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-lg px-4 py-3 cursor-pointer hover:border-slate-400 hover:bg-white transition-all group"
          >
...
export default function EditCoursePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    api.get(`/resources/${id}`)
      .then((res) => {
        ...
        const rebuilt: Section[] = Array.from(sectionMap.entries()).map(([title, lessons]) => ({
  ...
  const handleThumbnailUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    upload({ file, type: "thumbnail" }, {
      onSuccess: (res) => { setCourseInfo((p) => ({ ...p, thumbnail: res.url })); toast.success("Thumbnail updated!"); },
  ...
  const handleUploadFile = (sectionId: string, lessonId: string, file: File) => {
    ...
    const uploadType = isCode ? "thumbnail" : "video"; // use thumbnail type for raw files (no video transform)

    if (isCode) {
      // Read locally for preview, upload as raw
  ...
  const handleLessonDragEnd = (sid: string, event: DragEndEvent) => {
    ...
    setSections((p) => p.map((s) => {
      ...
      const oi = s.lessons.findIndex((l) => l.id === active.id);
      const ni = s.lessons.findIndex((l) => l.id === over.id);
      return { ...s, lessons: arrayMove(s.lessons, oi, ni) };
    }));
  };

  const totalLessons = sections.reduce((acc, s) => acc + s.lessons.length, 0);
  const canProceedStep1 = courseInfo.title.trim() && courseInfo.description.trim();
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      <div className="max-w-4xl mx-auto py-10 px-6">
        <Link href={`/creator/dashboard/${id}`} className="inline-flex items-center gap-2 text-[10px] font-bold uppercase text-slate-400 hover:text-slate-900 mb-8 transition-all">
          <FiArrowLeft size={13} /> Back to Course
        </Link>
        <div className="mb-8">
          <h1 className="text-3xl font-black uppercase tracking-tighter italic text-slate-900">Edit <span className="text-blue-600">Course</span></h1>
          <p className="text-slate-500 text-sm font-medium mt-1">Update your course info, curriculum, and lesson files.</p>
        </div>
        ...
        {step === 1 && (
          <div className="space-y-6 animate-in fade-in duration-300">
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 space-y-6">
              ...
              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-2">Price (NPR)</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm">NPR</span>
                  <input type="number" min={0} value={courseInfo.price || ""}
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-4">Course Thumbnail</label>
            ...
            <div className="flex justify-end">
              ...
              >
                Edit Curriculum <FiArrowRight size={14} />
              </button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <form onSubmit={handleSave} className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex items-center gap-5">
              ...
              <button type="button" onClick={() => setStep(1)} className="text-[10px] font-black uppercase text-slate-400 hover:text-slate-900 transition-colors tracking-wider flex items-center gap-1">
            ...
            <div className="space-y-4">
              {sections.map((section, i) => (
                <SectionBlock key={section.id} section={section} index={i}
                  onRename={renameSection} onRemove={removeSection}
                  onAddLesson={addLesson} onRemoveLesson={removeLesson}
                  onUpdateLesson={updateLesson} onDragEnd={handleLessonDragEnd}
                  uploadingId={uploadingId} onUploadFile={handleUploadFile}
                />
              ))}
            </div>
            ...
            <div className="flex items-center justify-between pt-2">
              <button type="button" onClick={() => setStep(1)} className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 hover:text-slate-900 transition-colors tracking-wider">
