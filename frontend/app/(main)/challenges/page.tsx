...
export default function ChallengesPage() {
  ...
  const { items, isLoading } = useSelector((state: RootState) => state.challenge);
  const [filter, setFilter] = useState<typeof DIFF_FILTERS[number]>("ALL");

  useEffect(() => { dispatch(fetchPublicChallengesThunk()); }, [dispatch]);

  const filtered = filter === "ALL" ? items : items.filter((c) => c.difficulty === filter);
  const completedCount = items.filter((c) => (c as any).stats?.isCompleted).length;
      ...
      <div className="max-w-6xl mx-auto py-10 px-6">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((c) => <ChallengeCard key={c.challengeId} challenge={c} />)}
          </div>
        ) : (
          <div className="bg-white rounded-xl border-2 border-dashed border-slate-200 py-20 text-center">
            ...
            <p className="text-slate-300 text-xs mt-1">Check back soon for new contests</p>
          </div>
        )}
      </div>
    </div>
