...
import { useCreateResourceMutation, useUpdateResourceMutation } from "@/hooks/useResource";
...
function SortableModule({ m, i, updateModuleTitle, removeModule }: any) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="group relative border border-slate-200 rounded-sm bg-white overflow-hidden shadow-sm">
      <div className="relative aspect-video bg-slate-900">
        <video src={m.localPreview || m.url} className="w-full h-full object-cover opacity-80" />
        <div {...attributes} {...listeners} className="absolute top-2 right-2 p-2 bg-white/90 rounded-sm cursor-grab active:cursor-grabbing hover:bg-white transition-colors shadow-sm z-10">
      ...
      <div className="p-5 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Chapter {i + 1}</span>
          <button type="button" onClick={() => removeModule(i)} className="p-2 text-slate-300 hover:text-rose-500 transition-colors">
            <FiTrash2 size={16} />
          </button>
        </div>
      </div>
...
export default function ResourceForm({ initialData, resourceId }: ResourceFormProps) {
  ...
  const [series, setSeries] = useState({
    ...
    modules: initialData?.modules?.map((m: any) => ({
      title: m.title,
      url: m.contentUrl,
      type: "video"
    })) || [],
  ...
  const { mutate: upload, isPending: isUploading, progress, variables } = useUploadMutation();
  
  const createMutation = useCreateResourceMutation();
  const updateMutation = useUpdateResourceMutation(resourceId || "");

  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>, type: "thumbnail" | "video") => {
    const file = e.target.files?.[0];
    if (!file) return;
    upload({ file, type }, {
      onSuccess: (res) => {
        if (type === "thumbnail") setSeries(p => ({ ...p, thumbnail: res.url }));
        else setSeries(p => ({ ...p, modules: [...p.modules, { title: file.name.replace(/\.[^/.]+$/, ""), url: res.url, type: "video" }] }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-6 bg-white p-8 border border-slate-200 rounded-sm shadow-sm">
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Resource Title</label>
            <input
              type="text"
              required
        ...
        <div className="space-y-6 bg-white p-8 border border-slate-200 rounded-sm shadow-sm">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Cover Art</label>
            <label className="relative flex flex-col items-center justify-center aspect-video border-2 border-dashed border-slate-100 rounded-sm cursor-pointer overflow-hidden group">
              {series.thumbnail ? (
                <img src={series.thumbnail} className="w-full h-full object-cover group-hover:scale-105 transition-all" />
              ) : (
                <FiImage className="text-slate-300" size={24} />
              )}
              <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white text-[10px] font-bold uppercase">Change Image</div>
      ...
      <div className="bg-white border border-slate-200 p-8 rounded-sm shadow-sm">
        <h3 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] mb-8">Course Curriculum</h3>
          ...
          <SortableContext items={series.modules.map((m) => m.url)} strategy={verticalListSortingStrategy}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              ...
              <label className="flex flex-col items-center justify-center aspect-video border-2 border-dashed border-slate-200 rounded-sm cursor-pointer hover:border-slate-900 transition-all">
                {isUploading && variables?.type === "video" ? <FiLoader className="animate-spin" /> : <FiPlus size={32} />}
      ...
      <div className="flex justify-end sticky bottom-8 z-20">
        ...
        >
          {(createMutation.isPending || updateMutation.isPending) ? <FiLoader className="animate-spin" /> : <><FiSave /> {isEdit ? "Update Series" : "Publish Series"}</>}
        </button>
      </div>
    </form>
  );
