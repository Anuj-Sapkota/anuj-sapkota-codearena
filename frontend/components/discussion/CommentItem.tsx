...
export const CommentItem = ({
  ...
  const renderFormattedContent = (
    ...
    if (isBlocked && !isAdmin) {
      return (
        <p className="text-sm text-slate-400 italic my-3">
    ...
    <div className={`flex flex-col w-full ${depth > 0 ? "mt-4" : "mb-5"}`}>
      ...
      >
        ...
        <div className="flex flex-col gap-4">
          ...
          <div
            className={`text-slate-700 ${isBlocked && !isAdmin ? "italic" : ""}`}
          >
            {isEditing ? (
              <DiscussionEditor
          ...
          <div className="flex items-center gap-3 pt-4 border-t-2 border-slate-50">
            <button
              ...
              disabled={isBlocked}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-200 bg-slate-50 hover:bg-slate-100 active:scale-95 disabled:opacity-50 ${comment.hasUpvoted ? "text-emerald-600" : "text-slate-400"}`}
            >
              {comment.hasUpvoted ? (
                <HiHandThumbUp
                  size={14}
            ...
            <button
              ...
              disabled={isBlocked}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer disabled:opacity-50 ${isReplying ? "bg-slate-900 text-white" : "bg-slate-50 text-slate-400 hover:bg-slate-100"}`}
            >
