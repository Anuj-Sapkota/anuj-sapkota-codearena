...
import { fetchChallengesThunk } from "@/lib/store/features/challenge/challenge.actions";
...
export const FeaturedChallenges = () => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchChallengesThunk({ page: 1, limit: 4 }));
  }, [dispatch]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {items.length > 0 ? (
          items.slice(0, 4).map((item) => (
            <ChallengeCard key={item.challengeId || item.id} challenge={item} />
          ))
        ) : (
