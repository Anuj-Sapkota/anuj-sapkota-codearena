 ...
 */
export interface SerializedFetchProblemsParams extends Omit<
  FetchProblemsParams,
  "categoryIds"
> {
  categoryIds?: string;
...
export const createProblemThunk = createAsyncThunk<
  ...
  CreateProblemDTO,
  { rejectValue: string }
>("problems/create", async (problemData, { rejectWithValue }) => {
  try {
    return await problemService.create(problemData);
  } catch (error: unknown) {
    return rejectWithValue(
      handleAxiosError(error) || "Failed to create problem",
    );
  }
});
...
>("problems/update", async ({ id, data }, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      handleAxiosError(error) || "Failed to update problem",
    );
  }
});
...
export const deleteProblemThunk = createAsyncThunk<
  ...
  number,
  { rejectValue: string }
>("problems/delete", async (id, { rejectWithValue }) => {
  try {
    const response = await problemService.delete(id);
    return { ...response, id };
  } catch (error: unknown) {
    return rejectWithValue(
      handleAxiosError(error) || "Failed to delete problem",
    );
  }
});

export const fetchProblemByIdThunk = createAsyncThunk<
  { success: boolean; data: Problem },
  string, // Change this to string to match your URL params
  { rejectValue: string }
>("problems/fetchById", async (id, { rejectWithValue }) => {
  try {
    // The service will hit /api/problems/:id
    return await problemService.getById(id);
  } catch (error: unknown) {
    return rejectWithValue(
      handleAxiosError(error) || "Failed to fetch problem details",
    );
  }
});
